          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-12">
            ...
            <div className="lg:col-span-2 space-y-6 sm:space-y-10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                {[
                  { label: 'Min Age', value: `${tour.minimumAge || '5'}+ Yrs`, icon: <Users className="w-4 h-4 sm:w-5 sm:h-5" /> },
                  { label: 'Travelers', value: 'Flexible', icon: <Users className="w-4 h-4 sm:w-5 sm:h-5" /> },
                  { label: 'Starting', value: tour.startingPlace || 'Airport', icon: <MapPin className="w-4 h-4 sm:w-5 sm:h-5" /> },
                  { label: 'Rating', value: `${(tour as any).rating || '4.9'} / 5`, icon: <Star className="w-4 h-4 sm:w-5 sm:h-5" /> }
                ].map((item, idx) => (
                  <div key={idx} className="bg-white p-3 sm:p-5 rounded-2xl border border-slate-100 shadow-sm">
                    ...
                    <div className="text-xs sm:text-sm font-bold text-primary leading-tight">{item.value}</div>
                  </div>
                ))}
              </div>
